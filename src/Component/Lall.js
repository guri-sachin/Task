...
const Home = () => {
  ...
  useEffect(()=>{
          axios.get("http://localhost:4200/listloc").then(res=>setLoc(res.data));
          axios.get("http://localhost:4200/liststream").then(res=>setStreams(res.data));
      },[]);
...
function Show(e){
 ...
 axios.post("http://localhost:4200/locationls",data).then(
        res=>  
        setCollege(res.data)
  )}
  ...
  function Shop(e){
    var data=  {"color":e.target.value}
    axios.post("http://localhost:4200/streamlist",data).then(
           res=>  
           setCollege(res.data)
     )}
